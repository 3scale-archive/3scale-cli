...
var activedocs_debug = require('debug')('3scale-cli:activedocs');
...
exports.listActiveDocs = function(){
  ...
  return response.then(function (r) {
    ...
    activedocs_debug('HTTP API call result: %o', body)
    if (res.statusCode >= 300) {
      cli.error({message:"ERROR encountered: " + JSON.stringify(body.error || body.status)});
      cli.debug({name: '3scale-cli:activedocs:list', msg: 'ERROR:' +r[0].statusCode+" "+ JSON.stringify(body.errors || body.status)})
      throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.error || body.status));
    } else {
      cli.debug({name: '3scale-cli:activedocs:list', msg: 'result: ' +JSON.stringify(body.api_docs)})
...
exports.createActiveDocs = function(file){
  ...
  .then(function(api){
    ...
    return response.then(function (r) {
      ...
      if (res.statusCode >= 300) {
        cli.error({message:"ERROR encountered: " + JSON.stringify(body.error || body.status)});
        cli.debug({name: '3scale-cli:activedocs:create', msg: 'ERROR:' +r[0].statusCode+" "+ JSON.stringify(body.errors || body.status)})
        throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.error || body.status));
      } else {
        cli.debug({name: '3scale-cli:activedocs:create', msg: 'result: ' +JSON.stringify(body.api_doc)})
        return body.api_doc
      }
    });
  })
...
exports.updateActiveDocs = function(file, id){
  ...
  .then(function(api){
    ...
    return response.then(function (r) {
      ...
      if (res.statusCode >= 300) {
        cli.error({message:"ERROR encountered: " + JSON.stringify(body.error || body.status)});
        cli.debug({name: '3scale-cli:activedocs:update', msg: 'ERROR:' +r[0].statusCode+" "+ JSON.stringify(body.errors || body.status)})
        throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.error || body.status));
      } else {
        cli.debug({name: '3scale-cli:activedocs:update', msg: 'result: ' +JSON.stringify(body.api_doc)})
        return body.api_doc
      }
    });
  });
...
exports.deleteActiveDocs = function(activedocs_id){
  ...
  return response.then(function (r) {
    ...
    if(r[0].body == ''){ //DELETE has no payload unless error encountered
      ...
      activedocs_debug('HTTP API call result: %o', body)
      cli.error({message:"ERROR encountered: " + JSON.stringify(body.error || body.status)});
      cli.debug({name: '3scale-cli:activedocs:delete', msg: 'ERROR:' +r[0].statusCode+" "+ JSON.stringify(body.errors || body.status)})
      throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.error || body.status));
    }
  })
}
