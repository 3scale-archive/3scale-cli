(function() {
 var cli, colors, err, pkg, program, parser, async, slug, request, generator;
...
program
  ...
  program
    ...
    program
      ...
      .description("create - Create a new mapping rule \n  list - List all mapping rules of a service \n  show - Show a specific mapping rulesof a service")
      ...
      .action(function(command,options){
        ...
        if(command === "create"){ //create a new mapping rule
          ...
          maprules.createMappingRule(options.service, options.http, options.pattern, options.delta, options.metric);
        }else if (command === "list") {
          if(!options.service){
            cli.error({message:"Service ID is required"});
            process.exit(1);
          }
          maprules.listMappingRules(options.service)
          ...
          maprules.getMappingRule(options.service,options.maprule)
          ...
          maprules.updateMappingRule(options.service,options.maprule,options.http, options.pattern, options.delta, options.metric)
          ...
          maprules.deleteMappingRule(options.service,options.maprule)
        }
      });
