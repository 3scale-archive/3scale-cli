...
exports.createMethod = function(service_id,metric_id,friendly_name, unit){
  ...
  return response.then(function (r) {
    ...
    if (res.statusCode >= 300) {
      cli.error({message:"ERROR encountered on creating Method: " + JSON.stringify(body.errors || body.status)});
...
exports.getMethodById = function(service_id, metric_id, method_id){
  ...
  return response.then(function (r) {
    ...
    var body = JSON.parse(r[0].body);
    if (res.statusCode >= 300) {
      cli.error({message:"ERROR encountered on retrieving Method: " + JSON.stringify(body.errors || body.status)});
      throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.errors || body.status));
    } else {
      return body.method;
    }
