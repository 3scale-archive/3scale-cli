...
exports.createMethod = function(service_id, metric_id, friendly_name, unit){
  ...
  return response.then(function (r) {
    ...
    if (res.statusCode >= 300) {
      ...
      cli.debug({name: '3scale-cli:methods:create', msg: 'ERROR:' +r[0].statusCode+" "+ JSON.stringify(body.errors || body.status)})
      throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.errors || body.status));
    } else {
      cli.debug({name: '3scale-cli:methods:create', msg: 'result: ' +JSON.stringify(body.method)})
      return body.method;
    }
  });
};
...
exports.listMethods = function(service_id, metric_id){
  ...
  return response.then(function (r) {
    ...
    if (res.statusCode >= 300) {
      ...
      cli.debug({name: '3scale-cli:methods:list', msg: 'ERROR:' +r[0].statusCode+" "+ JSON.stringify(body.errors || body.status)})
      throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.error || body.status));
    } else {
      cli.debug({name: '3scale-cli:methods:list', msg: 'result: ' +JSON.stringify(body.methods)})
      return body.methods;
    }
  });
}
...
exports.getMethodById = function(service_id, metric_id, method_id){
  ...
  return response.then(function (r) {
    ...
    if (res.statusCode >= 300) {
      ...
      cli.debug({name: '3scale-cli:methods:get', msg: 'ERROR:' +r[0].statusCode+" "+ JSON.stringify(body.errors || body.status)})
      throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.error || body.status));
    } else {
      cli.debug({name: '3scale-cli:methods:get', msg: 'result: ' +JSON.stringify(body.method)})
      return body.method;
    }
  });
}
...
exports.updateMethod = function(service_id, metric_id, method_id, friendly_name, unit){
  ...
  return response.then(function (r) {
    ...
    if (res.statusCode >= 300) {
      ...
      cli.debug({name: '3scale-cli:methods:update', msg: 'ERROR:' +r[0].statusCode+" "+ JSON.stringify(body.errors || body.status)})
      throw new Error("Server responded with status code " + r[0].statusCode + " "+JSON.stringify(body.error));
    } else {
      cli.debug({name: '3scale-cli:methods:update', msg: 'result: ' +JSON.stringify(body.method)})
      return body.method;
    }
  });
}
